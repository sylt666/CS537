...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  // sz    - size
  pde_t *d;       // kernel part of page table
  pte_t *pte;     // pte of the 
  uint pa, i;     // 
  char *mem;      // memeory of the child

  if((d = setupkvm()) == 0)
    return 0;
  // Going through the whole size of the file to be copied
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)
      goto bad;
    // Copy portion of memory from parent to the child
