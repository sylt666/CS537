...
{
   int pid_low = getpid();
   int lowtickets = 10, medtickets = 20;
   ...
   if(fork() == 0)
   {
   	check(settickets(medtickets) == 0, "settickets");
        int pid_med = getpid();
        struct pstat st_low,st_med;
        check(getpinfo(&st_low) == 0, "getpinfo");
	      printf(1, "\n ****PInfo low**** \n");
	      print(&st_low);
        printf(1,"Spinning...\n");
        ...
        check(getpinfo(&st_med) == 0, "getpinfo");
	      printf(1, "\n ****PInfo med**** \n");
	      print(&st_med);
	
        compare(pid_low, pid_med, &st_low, &st_med);
