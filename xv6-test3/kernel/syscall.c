...
{
  // UPDATE: addr should not be 0
  if (p->pid != 1 && addr < PGSIZE) return -1;
  if((addr >= p->sz || addr+4 > p->sz || addr == 0) && addr < p->ssz)
    return -1;
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  // UPDATE: addr == 0
  if (p->pid != 1 && addr < PGSIZE) return -1;
  if((addr >= p->sz || addr == 0) && addr < p->ssz)
    return -1;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  // UPDATE: i == 0
  if(((uint)i >= proc->sz || (uint)i+size > proc->sz || (uint)i < PGSIZE) && (uint)i < proc->ssz)
    return -1;
