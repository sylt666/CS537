...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  /*
    for (tot = 0; tot < n; tot++){
    	cprintf("((char *)ip->addrs)[tot + off] = 0x%x\n", ((char *)ip->addrs)[tot + off]);
    	dst[tot] = ((char *)ip->addrs)[tot + off];
    }
*/
	 memmove(dst, ((char *) ip->addrs) + off, (n > 52) ? 52 : n);
...
/*
    for (tot = 0; tot<n/4 + 1; tot++){
    	cprintf("writing 0x%x\n", ((uint *) src)[tot]);
      ip->addrs[tot] = ((uint *) src)[tot + off];
    }
*/
	 ...
	 memmove((char *) ip->addrs + off, src, (n >= 52) ? 52 : n);
