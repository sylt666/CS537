...
int shmem_count1[SHMEM_PAGES];
void *shmem_addr[SHMEM_PAGES];
void *shmem_addr_virt[SHMEM_PAGES];

int top_of_mem = USERTOP;
...
int shmem_count(int page_no) {
  
  return shmem_count1[page_no];
  
}

void *shmem_access(int page_no) {

  //  void *shmem_addr_virt[SHMEM_PAGES];

  void *ptr = NULL;

  if (proc->sz >= top_of_mem)
    return NULL;

  //  if (proc->page_shared[page_no] == NULL)
  //return NULL;

  if (shmem_addr_virt[page_no] != NULL)
    return shmem_addr_virt[page_no];
  
  top_of_mem -= PGSIZE;

  int ret = mappages(proc->pgdir, (void *) top_of_mem, PGSIZE, (int) shmem_addr[page_no], PTE_W|PTE_U);
  ...
  shmem_addr_virt[page_no] = ptr;

  shmem_count1[page_no] = shmem_count1[page_no] + 1;
  
  proc->page_shared[page_no] = 0;
...
// exit() -> address space is freed
// follow the wait() code
// make sure NOT to free SHARED pages
// fork() ->
// have to be careful w/ shared pages
// exec() -> also an interesting case
// heap grows, may destory world - be careful
